...
<div id="account_info_{$block.snapping_id}">
    ...
    <div class="buttons-container">
        {if $auth.user_id}
            <a href="{"profiles.update"|fn_url}" rel="nofollow" class="underlined">MY ACCOUNT</a>     |     <a href="{"auth.logout?redirect_url=`$return_current_url`"|fn_url}" rel="nofollow" class="account">SIGN OUT</a>
        {else}
            <a href="{if $runtime.controller == "auth" && $runtime.mode == "login_form"}{$config.current_url|fn_url}{else}{"auth.login_form?return_url=`$return_current_url`"|fn_url}{/if}" {if $settings.General.secure_auth != "Y"} data-ca-target-id="login_block{$block.snapping_id}" class="cm-dialog-opener cm-dialog-auto-size account"{else}rel="nofollow" class="account"{/if}>LOGIN</a>     |     <a href="{"profiles.add"|fn_url}" rel="nofollow" class="account">SIGNUP</a>
